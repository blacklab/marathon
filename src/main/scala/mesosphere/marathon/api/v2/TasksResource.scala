...
class TasksResource @Inject() (
  ...
  @Timed
  def indexTxt(@Context req: HttpServletRequest): Response = doIfAuthenticated(req) { implicit identity =>
  ...
  def killTasks(
    ...
    doIfAuthenticatedAndAuthorized(req, UpdateApp, affectedApps: _*) {
